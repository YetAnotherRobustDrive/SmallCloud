...
class LogControllerTest {
    ...
    @Nested
    @DisplayName("/logs/count document")
    class count{
        private final String url = URL_PREFIX + "/count";
        private UserLog log;
        private RequestLogDto requestLogDto;
        ...
        void boot() {
            ...
            log = UserLog.of(member, testTime.plusHours(0), "/ping/login/testMemberName/success", "111.111.111.111", false);
            em.persist(log);
            log = UserLog.of(member, testTime.plusHours(0), "/ping/login/testMemberName/success", "111.111.111.111", false);
            em.persist(log);
            log = UserLog.of(member, testTime.plusHours(0), "/ping/login/testMemberName/fail", "111.111.111.111", false);
            em.persist(log);

            em.flush();
        }

        @DisplayName("어드민 로그 개수 조회")
        @Test
        void getAdminCount() throws Exception {
            String urlAdmin = url + "/admin?size=200&page=0";
            mockMvc.perform(TestSnippet.secured(get(urlAdmin), adminToken.getAccessToken()))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andDo(document(DOCUMENT_NAME));
        }

        @DisplayName("유저 로그인 로그 개수 조회")
        @Test
        void getUserCount() throws Exception {
            mockMvc.perform(TestSnippet.secured(get(url), memberToken.getAccessToken()))
                    ...
                    .andExpect(status().isOk())
                    .andExpect(rst -> {
                        Integer res = objectMapper.readValue(rst.getResponse().getContentAsString(), Integer.class);
                        assertEquals(3, res);
                    })
                    .andDo(document(DOCUMENT_NAME));
        }
